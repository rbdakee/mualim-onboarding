...
IP=127.0.0.1
PORT=5000
SECRET_TOKEN_API=ваш_токен
```

> Примечание: фронт отправляет этот токен в заголовке `X-API-TOKEN` при запросах к бэкенду.
> Для обратной совместимости также поддерживаются `X_API_TOKEN` / `API_TOKEN`.
